...
class RepeatReplacer(object):
    ...
    def replace(self, word):
        if wordnet.synsets(word): # normal words
            return word

        repl_word = self.repeat_regexp.sub(self.repl, word) # replace repetition

        if repl_word != word: # if replaced word cannot be found in the dictionary of normal words
            return self.replace(repl_word) # replace again
        else:
            return repl_word # once the replace word is matched with a normal word, the replaced word is returned

######################################
## Spelling Correctiont ##
######################################
